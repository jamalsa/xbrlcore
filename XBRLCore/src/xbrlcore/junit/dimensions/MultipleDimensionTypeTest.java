 ...
 * @author Daniel Hamm
 */
public class MultipleDimensionTypeTest extends TestCase {

	DiscoverableTaxonomySet stTaxonomy;

	TaxonomySchema laTaxonomy;

	TaxonomySchema koTaxonomy;

	TaxonomySchema prTaxonomy;

	String PATH = "xbrl\\test\\taxonomy_typed_dimension\\";
...
//			 stTaxonomy = taxonomyFactory.createTaxonomy(stTaxonomyFile);
			...
			laTaxonomy = stTaxonomy.getTaxonomySchema("d-la-2006-12-31.xsd");
			koTaxonomy = stTaxonomy.getTaxonomySchema("d-ko-2006-12-31.xsd");
			prTaxonomy = stTaxonomy.getTaxonomySchema("p-pr-2006-12-31.xsd");
		} catch (Exception ex) {
	 ...
	 * 
	 */
	public void testMultipleDimensionType() {
		try {
			...
			/*
			 * there must be only *one* predecessor dimension / domain
			 * combination, since both dimension / domain combinations which
			 * have been added refer to the same dimension!
			 */
	...
	public void testSingleDimensionType() {
		try {
			Concept laenderDimension = stTaxonomy
					.getConceptByID("d-la_LaenderDimension");
			Concept landDeutschland = stTaxonomy
